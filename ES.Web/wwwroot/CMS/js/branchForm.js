...
$(document).ready(function () {
    initializeSelect2('#parentBranchDropdown');

    // Determine the placeholder language based on the current language
    if (currentLanguage === 'ar-JO') {
        initializeSelect2('#relatedBranchesDropdown', 'حدد الفروع ذات الصلة');
    } else if (currentLanguage === 'en-JO') {
        initializeSelect2('#relatedBranchesDropdown', 'Select related Branchss');
    }
            ...
            if (data.success) {
                Swal.fire({
                    ...
                    if (data.id) {
                        ...
                        //window.location.href = `${baseUrlWithSlash}EsAdmin/Branches/Edit/${data.id}`;
                        window.location.href = `${baseUrlWithSlash}EsAdmin/BranchesTranslates?branchId=${data.id}`;
                    }
                });
            } else {
                Swal.fire({
