...
$(document).ready(function () {
    ...
    $(document).on('click', '.toggle-status', function () {
        ...
        toggleBranchStatus(branchId, this);
    });

});

async function deleteBranch(branchId, element) {
    console.log('deleteBranch called for ID:', branchId); // Debug
    ...
    try {
        const DeleteUrl = `${baseUrl}EsAdmin/Branchs/Delete?id=${branchId}`;
        // Send DELETE request
        ...
        if (response.ok) {
            ...
            Swal.fire({
                ...
                text: currentLanguage === 'ar-JO' ? "تم حذف المادة بنجاح." : "Branch deleted successfully.",
                icon: "success",
                timer: 3000
            });
            ...
            console.error('Failed to delete Branch:', responseText);
            Swal.fire({
                title: currentLanguage === 'ar-JO' ? "خطأ!" : "Error!",
                text: currentLanguage === 'ar-JO' ? "!! لا يمكن حذف هذه المادة رئيسية أو مرتبطة بمادة أخرى" : "Can't delete this Parent / Related to another branch!",
                icon: "error",
                timer: 3000
            });
            ...
            Swal.fire({
                ...
                text: currentLanguage === 'ar-JO' ? "ليس لديك إذن لحذف هذه المادة!" : "You don't have permission to delete this branch!",
                icon: "error",
                timer: 3000
            });
        ...
        Swal.fire({
            title: currentLanguage === 'ar-JO' ? "خطأ!" : "Error!",
            text: currentLanguage === 'ar-JO' ? "حدث خطأ أثناء حذف المادة. تحقق من اتصالك وحاول مرة أخرى." : "An error occurred while deleting the branch. Please check your connection and try again.",
            icon: "error",
            timer: 3000
        });
    }
...
async function toggleBranchStatus(branchId, element) {
    try {
        const ToggleUrl = `${baseUrl}EsAdmin/Branches/ToggleStatus?id=${branchId}`;
        // Send the request
        ...
        if (response.status === 200) {
            ...
            Swal.fire({
                ...
                text: currentLanguage === 'ar-JO' ? "تم تحديث حالة المادة بنجاح!" : "Branch status updated successfully!",
                icon: "success",
                timer: 2000,
            ...
            Swal.fire({
                title: currentLanguage === 'ar-JO' ? "ممنوع!" : "Forbidden!",
                text: currentLanguage === 'ar-JO' ? "ليس لديك إذن لتعديل حالة هذه المادة!" : "You don't have permission to edit this branch!",
                icon: "error"
            });
        } else {
            // Show generic error
            Swal.fire({
                title: currentLanguage === 'ar-JO' ? "خطأ!" : "Error!",
                text: currentLanguage === 'ar-JO' ? "تعذر تعديل حالة المادة!" : "Failed to update branch status!",
                icon: "error"
            });
        }
    } catch (error) {
